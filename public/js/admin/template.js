$(document).ready(function() {
    ...
    function load_table()
    {
        ...
        $("table#data_template").DataTable().destroy();
        $("table#data_template").DataTable({
            ajax: {
                url: link_api,
                headers: {
            ...
            columns: [
                ...
                { data: "name", name: "name" },
                {
                    ...
                    render: (res) => {
                        ...
                        let link_image = link+"/"+res.image_template;
                        return `<a href="${link_image}" target="_blank" title="lihat gambar ukuran penuh" class="view_image"><img src="${link_image}" width="50"></a>`
                    }
                },
                {
                    data: null,
                ...
                { data: "count_select", name: "count_select" },
                {
                    data: null,
                    render: (res) => {
                        return `
                        <a href="#" class="btn btn-primary btn-sm btn-edit" data-id="${res.id}" data-name="${res.name}" data-image="${res.image_template}" data-link="${res.link}" style="color: #FFF;" title="edit"><i class="fa fa-edit"></i></a>
                        <button class="btn btn-danger btn-sm btn-hapus" data-id="${res.id}" style="color: #FFF;" title="hapus"><i class="fa fa-trash"></i></button>
                        `;
                    },
                },
            ],
    ...
    $('#form_addTemplate').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if(res.success) {
                    ...
                    if(res.info_error == 'validasi_eror') {
                        validasi(res.errors);
                    } else {
                        Swal.fire({
                            icon: 'error',
    ...
    $(document).on('click', '.btn-edit', function() {
        ...
        let image_template = $(this).attr('data-image');
        let link_html = $("meta[name='link-image']").attr("link");
        let link_image = link_html+"/"+image_template;
        $('input#id_edit').val(id);
        $('input#name_edit').val(name);
        $('input#link_edit').val(link);
        $('input#old_image').val(image_template);
    ...
    $(document).on('click', '.btn-hapus', function() {
        ...
        Swal.fire({
            ...
            text: "Data template undangan akan dihapus!",
            icon: 'warning',
